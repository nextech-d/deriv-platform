...
export function bulkDefaultDigit(_type: BulkTradeFamily): number {
  return 5;
}
...
export function bulkWinRates(
  pcts: number[],
  digit: number,
): Record<BulkContract, number> {
...
export function bulkMartingaleStake(
  current: number,
  won: boolean,
  risk: BulkAutoRisk,
): number {
