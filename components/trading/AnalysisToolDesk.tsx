...
function marketMeta(id: string) {
  return DERIV_SYMBOLS.find((s) => s.id === id);
}

export function AnalysisToolDesk({
  symbol,
  quotes,
  onSymbolChange,
  onTradeBias,
  ...
  const liveDigits = useMemo(() => {
    ...
    return digitsFromQuotes(series, ticks);
  }, [quotes, ticks, symbol, tagged]);
  ...
  const barrierStats = useMemo(
    () => analyzeBarrier(liveDigits, barrier),
    [barrier, liveDigits],
  );
  const matches = useMemo(
    () => analyzeMatches(liveDigits, matchTarget),
    [liveDigits, matchTarget],
  );
  ...
  const marketRows = useMemo(
    () =>
      DCIRCLE_SYMBOLS.map((id) => {
        const series = tagged
          ? quotes.filter((tick) => tick.symbol === id)
          : id === symbol
            ? quotes
            : [];
        const freq = analyzeFrequency(digitsFromQuotes(series, ticks));
  ...
  function toggleScanAll() {
    setScanning((prev) =>
      prev.size === DCIRCLE_SYMBOLS.length ? new Set() : new Set(DCIRCLE_SYMBOLS),
    );
  }
  ...
  function currentBias(): AnalysisBias {
    if (mode === "parity") {
      ...
      return {
        ...
        label: evenLeads
          ? `Even ${parity.evenPct.toFixed(1)}%`
          : `Odd ${parity.oddPct.toFixed(1)}%`,
      };
    }
    if (mode === "barrier") {
    ...
    const matchPct =
      mode === "frequency"
        ? analyzeMatches(liveDigits, target).matchPct
        : matches.matchPct;
    return {
      mode: "matches",
      side: "CALL",
  ...
  const symbolLabel = marketMeta(symbol)?.label ?? symbol;
    ...
    <div data-testid="analysis-tool-desk" data-desk className="analysis-tool" data-scroll-pane>
        ...
        <div className="analysis-tool-segment">
          {(["dcircle", "analysis"] as const).map((id) => (
            <button
              key={id}
              type="button"
              className={cn("analysis-tool-seg", view === id && "is-on")}
              onClick={() => setView(id)}
            >
      ...
      <div className="analysis-tool-subbar">
        ...
        <div className="analysis-tool-windows">
          {TICK_WINDOWS.map((n) => (
            <button
              key={n}
              type="button"
              className={cn("analysis-tool-win", ticks === n && "is-on")}
              onClick={() => setTicks(n)}
            >
              {n}
            </button>
        ...
        <div className="analysis-tool-subbar-end">
          <span className="analysis-tool-chip is-signal">
            Signals · {strongCount} strong
          </span>
          {view === "dcircle" ? (
            <button
              type="button"
              className={cn("analysis-tool-cta", allScanning ? "is-ghost" : "is-ink")}
            ...
            return (
              ...
              >
                <header>
                  ...
                  <button
                    type="button"
                    className={cn("analysis-tool-scan", scanningNow && "is-on")}
                    onClick={() => toggleScan(row.id)}
                  >
                ...
                <div className="analysis-tool-digits">
                  {row.counts.map((count, digit) => (
                    ...
                    >
                      <span className={cn(ranks[digit] && `is-${ranks[digit]}`)}>{digit}</span>
        ...
        <div className="analysis-tool-desk">
          <div className="analysis-tool-desk-main">
            <div className="analysis-tool-modes">
              {MODES.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  className={cn("analysis-tool-mode", mode === item.id && "is-on")}
            ...
            <p className="analysis-tool-summary">
              ...
              {parity.oddPct.toFixed(1)}% · Over {barrier} {barrierStats.overPct.toFixed(1)}% ·
              Match {matchTarget} {matches.matchPct.toFixed(1)}%
            </p>
              ...
              {strip.length ? (
                strip.map((sample, i) => (
                  <span
                    ...
                    className={cn(sample.digit % 2 === 0 ? "is-even" : "is-odd")}
                  >
                    {sample.digit}
                  </span>
                ))
            ...
            <div className="analysis-tool-stats">
              {mode === "parity" ? (
                <>
                  <Stat label="Even" value={`${parity.evenPct.toFixed(1)}%`} sub={`${parity.evenCount} ticks`} />
                  <Stat label="Odd" value={`${parity.oddPct.toFixed(1)}%`} sub={`${parity.oddCount} ticks`} />
                  <Stat
                    label="Streak"
