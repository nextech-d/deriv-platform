...
export function TradesDrawer({
  ...
  return (
    <aside className="trades-drawer" data-testid="trades-drawer" aria-label="Trades">
      <div className="trades-drawer-tabs">
        {TABS.map((item) => (
          <button
            key={item}
            type="button"
            className={cn("trades-drawer-tab", tab === item && "is-on")}
          ...
          <div className="trades-drawer-summary">
            {contracts.length === 0 ? (
              <p className="trades-drawer-empty">
                When you&apos;re ready to trade, place a ticket. You&apos;ll be able to track performance here.
              </p>
            ) : null}
            <dl>
              {[
                ...
                <div key={label}>
                  <dt>{label}</dt>
                  <dd className={label === "Total profit/loss" ? (totalPnl >= 0 ? "is-up" : "is-down") : undefined}>
                    {value}
                  </dd>
                </div>
              ))}
