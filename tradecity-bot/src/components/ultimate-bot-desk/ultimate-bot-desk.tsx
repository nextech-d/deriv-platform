...
import './ultimate-bot-desk.scss';

export interface UltimateTick {
    quote: number;
    epoch: number;
    symbol: string;
}
...
const UltimateBotDesk = ({
    ...
    const limitStop = (tp > 0 && stats.profit >= tp) || (sl > 0 && stats.profit <= -sl);
    ...
    const bySymbol = useMemo(() => {
        const map = new Map<string, UltimateTick[]>();
        for (const market of ULTIMATE_BOT_MARKETS) {
            map.set(market.id, ticksForMarket(tickHistory, market.id));
        }
        return map;
    }, [tickHistory]);
    ...
    const progress = pendingTicket
        ? pendingProgress(bySymbol.get(pendingTicket.symbol) ?? [], pendingTicket.epoch, pendingTicket.duration)
        : null;
    const focused = pendingTicket?.symbol ?? armedTicket?.marketId ?? symbol ?? rows[0]?.market.id;
                ...
                <div className='edging-body'>
                        ...
                        <div className='edging-fields'>
                            ...
                            <label>
                                ...
                                <input
                                    ...
                                    max={10}
                                    value={duration}
                                    onChange={event => setDuration(clampUltimateDuration(Number(event.target.value)))}
                                />
                                <em>1–10 ticks</em>
                            </label>
                        </div>
                        ...
                        <div className='edging-fields'>
                            <label>
                                ...
                                <input
                                    ...
                                    step={0.05}
                                    value={stake}
                                    onChange={event => setStake(clampUltimateStake(Number(event.target.value)))}
                                />
                    ...
                    <section className='edging-card ultimate-bot__markets'>
                        ...
                        <p className='ultimate-meta'>
                            Subscribed {isConnected ? ULTIMATE_BOT_MARKETS.length : liveCount}/
                            {ULTIMATE_BOT_MARKETS.length}
                            {' · '}Live {liveCount}
                            {lastEpoch ? ` · Last tick ${formatTickTime(lastEpoch)}` : null}
                        </p>
                                ...
                                <tbody>
                                    {filtered.map(row => {
                                        ...
                                        return (
                                            <tr
                                                key={row.market.id}
                                                className={classNames({ 'is-on': row.market.id === focused })}
                                            ...
                                            >
                                                ...
                                                <td>
                                                    <span
                                                        className={classNames('ultimate-flag', {
                                                            'is-yes': row.initial.over,
                                                        })}
                                                    >
...
export default UltimateBotDesk;
