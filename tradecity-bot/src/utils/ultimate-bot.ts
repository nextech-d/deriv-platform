...
import type { TickPoint } from './tick-series';
...
export const ULTIMATE_MIN_STAKE = 0.35;
export const ULTIMATE_PAYOUT = 0.95;
export const ULTIMATE_MAX_DURATION = 10;
...
export const ULTIMATE_RECOVERY: UltimateRecovery[] = [
    {
        id: 'EO_PATTERN_CONTINUE',
        label: 'Even / Odd · continue current trend',
        family: 'even_odd',
        mode: 'pattern',
        invert: false,
    },
    {
        id: 'EO_PATTERN_REVERSAL',
        label: 'Even / Odd · reverse current trend',
        family: 'even_odd',
        mode: 'pattern',
        invert: true,
    },
    ...
    {
        id: 'RF_PATTERN_FOLLOW',
        label: 'Rise / Fall · follow current direction',
        family: 'rise_fall',
        mode: 'pattern',
        invert: false,
    },
    {
        id: 'RF_PATTERN_REVERSE',
        label: 'Rise / Fall · reverse current direction',
        family: 'rise_fall',
        mode: 'pattern',
        invert: true,
    },
    ...
    {
        id: 'OU_PATTERN_CONTINUE',
        label: 'Over / Under · continue current direction',
        family: 'over_under',
        mode: 'pattern',
        invert: false,
    },
    {
        id: 'OU_PATTERN_REVERSE',
        label: 'Over / Under · reverse current direction',
        family: 'over_under',
        mode: 'pattern',
        invert: true,
    },
    {
        id: 'OU_PCT_CONTINUE',
        label: 'Over / Under · strongest side',
        family: 'over_under',
        mode: 'percent',
        invert: false,
    },
...
export function clampUltimateDuration(value: number): number {
    if (!Number.isFinite(value)) return 1;
    return Math.max(1, Math.min(ULTIMATE_MAX_DURATION, Math.round(value)));
}

export function clampUltimateStake(value: number): number {
    if (!Number.isFinite(value)) return ULTIMATE_MIN_STAKE;
    return Math.max(ULTIMATE_MIN_STAKE, Math.round(value * 100) / 100);
}
...
export function ultimateStake(base: number, losses: number, martingale: boolean, multiplier: number): number {
    const sized = martingale
        ? clampUltimateStake(base) * clampMartMultiplier(multiplier) ** Math.min(Math.max(0, losses), 4)
        : clampUltimateStake(base);
...
/** Tick-clock label used by the Active Markets meta line. */
export function formatTickTime(epoch: number): string {
